...
public class ExternalProcesses{
	...
	//We assume both java card 2.2.1 and class files are in the resource directory
	public static void runJavaCardConverter() throws Exception{
		
		
		
		String config = "-config";
	    String optfile = "EmptyEidCard.opt";
		String converter;
		...
		if(System.getProperty("os.name").startsWith("Windows")){
			...
			String[] text = FileUtils.readTextFile(optfile);
			for (int i = 0;i<text.length;i++){
				text[1].replace("/", File.separator);
			}
			FileUtils.writeTextFile(text, optfile);
		}
		else converter = "java_card_kit-2_2_1"+File.separator+"bin"+File.separator+"converter";
