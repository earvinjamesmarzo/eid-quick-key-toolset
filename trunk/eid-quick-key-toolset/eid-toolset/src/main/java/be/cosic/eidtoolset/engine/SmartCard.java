...
import be.cosic.eidtoolset.interfaces.SmartCardCommandsInterface;
...
public class SmartCard implements SmartCardCommandsInterface {
	...
	public byte[] readBinaryFile(byte[] selectFileCommand)
			throws InvalidResponse, NoCardConnected, CardException {
		int wordLength = 1;
		...
		while (enough < 0) {
			...
			if ((keyBytes[keyBytes.length - 2] == (byte) 0x90)
				...
				//System.out.println("length of response data: " + len);
				
			} else {
				//does not get in here: TODO check if necessary: else throw invalidresponse!!!!
				enough = 0;
				if (keyBytes[keyBytes.length - 2] == (byte) 0x6c) {
					...
					if ((keyBytes[keyBytes.length - 2] == (byte) 0x90)
						...
						for (int j = 0; j < keyBytes.length - 2; j++)
							certificate[len + j] = keyBytes[j];
						len += keyBytes.length - 2;
						System.out.println("length of latest response data: " + (keyBytes.length - 2));
					} else {
	...
	public byte[] writeBinaryFile(byte[] selectFileCommand, byte[] data) throws InvalidResponse, NoCardConnected, CardException, GeneralSecurityException{
		...
		//Start selecting the File to write. Returns APDU response. Should be 0x9000
		byte[] keyBytes = myReader.sendCommand(selectFileCommand);
		...
		//NOTE only 255 bytes of data can be send in one APDU (on top of the 5 byte header)
    	...
    	}else apdusLeft = (data.length / (255)) + 1;
    	
		for(int i=0; i< apdusLeft; i++){
			offset = i*255;
			if (i == apdusLeft-1 && (data.length) % (255) != 0)
				writeDataLength = (data.length) % (255);
			else writeDataLength = 255;
			
			//NOTE: P1 and P2 should contain the offset in the file where the currently send bytes should be stored
			//Lc contains the number of bytes send and can not be bigger than the available bytes in the file, starting at the offset
			//This means that sending data that is to long for the selected File will result in an exception thrown: illegal length
			//TODO: check what about if data shorter then file length?
			...
			//check if the card is writable (ISO7816.SW_SECURITY_STATUS_NOT_SATISFIED)
