...
public class EidCard extends javacard.framework.Applet {
	...
	//Card ATR
	private final static byte[] ATR = {(byte)0x3B, (byte)0x98, (byte)0x13, (byte)0x40, (byte)0x0A, 
		(byte)0xA5, (byte)0x03, (byte)0x01, (byte)0x01, (byte)0x01, (byte)0xAD, (byte)0x13, (byte)0x11};
	...
	//INS for changing ATR: not available in original belpic eid applet.
	private final static byte INS_CHANGE_ATR = (byte)0x11;
	...
	// last nible of SW2 needs to be overwriten by the counter value/number of 
	// PIN tries left
	...
	private final static byte OFFSET_SECOND_PIN_HEADER =
		ISO7816.OFFSET_CDATA + 8;
	private final static byte OFFSET_SECOND_PIN_DATA = ISO7816.OFFSET_CDATA + 9;
	private final static byte OFFSET_SECOND_PIN_DATA_END =
		ISO7816.OFFSET_CDATA + 15;
	...
	private final static byte[] PKCS1_SHA1_HEADER =
		{
			(byte)0x00,
			(byte)0x30,
			(byte)0x21,
			...
			(byte)0x04,
			(byte)0x14 };
	private final static byte[] PKCS1_MD5_HEADER =
		{
			(byte)0x00,
			(byte)0x30,
			(byte)0x20,
	...
	protected static KeyPair tempkp;
	 ...
	 * see Belgian Electronic Identity Card content
	 */
	...
	private void clear() {
		...
		/*
		 * clear text message buffer, signature and previous ADPU type  are transient
		 * so no need to reset these manually
		 */
	}
	...
	public void process(APDU apdu) {
		...
		/*
		 * - non repudiation signatures can only be generated if the previous APDU 
		 * verified the cardholder PIN 
		 * - administrator PIN change is only possible if the previous APDU verified 
		 * the reset PIN
		 * 
		 * so only the "generate signature" and PIN Change APDU needs to check 
		 * the previous APDU type; in all other cases overwrite the previous APDU 
		 * type, because this information is not needed; we do this as early as 
		 * possible to cope with exceptions being thrown during processing of APDU
		 * 
		 * IMPORTANT : we have to set the previous APDU type in the processing of 
		 * a PIN Verify APDU (because the type gets overwritten to a wrong value)
		 * and at the end of a "generate signature" and PIN Change APDU
		 */
		if ((buffer[ISO7816.OFFSET_INS] != INS_GENERATE_SIGNATURE)
			&& (buffer[ISO7816.OFFSET_INS] != INS_CHANGE_PIN)
		...
		// verify the CLA byte
		if (buffer[ISO7816.OFFSET_CLA] != EIDCARD_CLA)
	...
	private void checkPin(OwnerPIN pin, byte[] buffer) {
		 ...
		 * the status word is of the form 0x63Cx with x the number of tries left
		 */
	...
	private void changePin(APDU apdu, byte[] buffer) {
		...
		/*
		 * IMPORTANT: in all other APDUs the previous APDU type gets 
		 * overwritten in process() function; this is not the case here 
		 * because the information is needed when processing to verify 
		 * the security condition for administrator PIN change
		 * 
		 * the previous APDU type has to be overwritten in every possible 
		...
		// P1 determines whether it is user or adminstrator PIN change
	...
	private void administratorChangePin(APDU apdu, byte[] buffer) {
		...
		if ((!resetPin.isValidated())
			|| (getPreviousApduType() != VERIFY_RESET_PIN)) {
			setPreviousApduType(OTHER);
	...
	private boolean isNewPinFormattedCorrectly(byte[] buffer, byte offset) {
		...
		// 1st nible of new PIN header should be 2
		if ((buffer[offset] >> 4) != 2)
			return false;

		// 2st nible of new PIN header is the length (in digits)
		...
		// if PIN length is odd, check if last PIN data nible is F
		if ((pinLength & (byte)0x01) == (byte)0x01) {
	...
	private boolean isNewPinCorrectValue(byte[] buffer) {
		...
		// 2st nible of the PIN header is the length (in digits)
		...
		if (oldLength == (byte)0x01) {
			if ((buffer[OFFSET_PIN_DATA + pinLengthInBytes] >> 4)
				!= ((randomBuffer[i] & 0x7F) >> 4))
				return false;
		}
	...
	private void prepareForSignature(APDU apdu, byte[] buffer) {
		...
		// check P1 and P2
		if (buffer[ISO7816.OFFSET_P1] != (byte)0x41
			|| buffer[ISO7816.OFFSET_P2] != (byte)0xB6)
		...
		if ((buffer[ISO7816.OFFSET_CDATA] != (byte)0x04)
			|| (buffer[ISO7816.OFFSET_CDATA + 1] != (byte)0x80)
	...
	private void generateSignature(APDU apdu, byte[] buffer) {
		 ...
		 * information is needed when processing to verify the security condition
		 * for non repudiation signature
		 * 
		...
		// check P1 and P2
		if (buffer[ISO7816.OFFSET_P1] != (byte)0x9E
			|| buffer[ISO7816.OFFSET_P2] != (byte)0x9A) {
			setPreviousApduType(OTHER);
			ISOException.throwIt(ISO7816.SW_WRONG_P1P2);
		}

		// generate signature without prepare signature results in
		 ...
		 * throw "security condition not satisfied" is something is wrong
		 */
		// check if previous APDU did a cardholder PIN verification
		if ((getSignatureType() == NON_REPUDIATION)
			&& (getPreviousApduType() != VERIFY_CARDHOLDER_PIN)) {
			setPreviousApduType(OTHER);
			ISOException.throwIt(ISO7816.SW_SECURITY_STATUS_NOT_SATISFIED);
		}
		// overwrite previous ADPU type as soon as possible
		...
		if (APDU.getProtocol() == APDU.PROTOCOL_T1) {
			Util.arrayCopy(
				...
				(short)0,
				(short)128);
			// in case T=0 protocol, send the signature immediately in a response APDU
		} else {
	...
	private void generatePkcs1Md5Signature(APDU apdu, byte[] buffer) {
		...
		if (getSignatureType() == AUTHENTICATION)
			cipher.init(authPrivateKey, Cipher.MODE_ENCRYPT);
	 ...
	 */
	private void preparePkcs1ClearText(
		byte[] clearText,
		short type,
		short messageLength) {
		...
		Util.arrayCopy(
			...
			(short)0,
			messageBuffer,
			(short) (128 - messageLength - header.length),
			(short)header.length);
	...
	private void generateKeyPair(APDU apdu) {
		...
		apdu.setIncomingAndReceive();//If this was removed, function will not work: no data except for command will be read
		byte[] buffer = apdu.getBuffer();
		...
		//short keyLength = Util.makeShort(buffer[ISO7816.OFFSET_CDATA], buffer[offset]);
		...
		tempkp = new KeyPair(KeyPair.ALG_RSA_CRT, (short)(1024));		
		tempkp.genKeyPair();
		
		//This is commented out as changing exponent makes getting modulus impossible on some java cards
		//((RSAPublicKey)tempkp.getPublic()).setExponent(buffer, (short)(13), (short)3);
		...
		switch(buffer[ISO7816.OFFSET_P2]){
			case BASIC :
				basicKeyPair = tempkp;
				tempkp.getPrivate().clearKey();
				tempkp = null;
				break;
			case AUTHENTICATION : // use authentication private key
				
				authPrivateKey = (RSAPrivateCrtKey) tempkp.getPrivate();
				tempkp.getPrivate().clearKey();
				
				break;
			case NON_REPUDIATION : // use non repudiation private key
				nonRepPrivateKey = (RSAPrivateCrtKey) tempkp.getPrivate();
				tempkp.getPrivate().clearKey();
				break;
			default :
				tempkp.getPrivate().clearKey();
	...
	private void getPublicKey(APDU apdu) {
		...
		byte[] buffer = apdu.getBuffer();
		//if this is thrown: problem acceesss getPreviousapdu
		...
		if (buffer[ISO7816.OFFSET_P2] == AUTHENTICATION || buffer[ISO7816.OFFSET_P2] == NON_REPUDIATION){
			...
			((RSAPublicKey) tempkp.getPublic()).getExponent(tempBuffer, (short)7);
			((RSAPublicKey) tempkp.getPublic()).getModulus(tempBuffer, (short)13);
	 ...
	 * as keys are always active, throw exception
	 */
	...
	private void internalAuthenticate(APDU apdu, byte[] buffer) {
		...
		// check P1 and P2
		if ((buffer[ISO7816.OFFSET_P1] != ALG_SHA1_PKCS1)
			|| buffer[ISO7816.OFFSET_P2] != BASIC)
			ISOException.throwIt(ISO7816.SW_WRONG_P1P2);
		...
		// the first data byte must be "94" and the second byte is the length (20 bytes)
		if ((buffer[ISO7816.OFFSET_CDATA] != (byte)0x94)
			|| (buffer[ISO7816.OFFSET_CDATA + 1] != (byte)0x14))
			ISOException.throwIt(ISO7816.SW_WRONG_DATA);
		...
		if (APDU.getProtocol() == APDU.PROTOCOL_T1) {
			Util.arrayCopy(
				...
				(short)0,
				(short)128);
			// in case T=1 protocol, send the signature immediately in a response APDU
		} else {
			// send first 128 bytes (= 1024 bit) of buffer
			apdu.setOutgoingAndSend((short)0, (short)128);
		}
	...
	private void selectByFileIdentifier(APDU apdu, byte[] buffer) {
		...
		// otherwise the current selected file is an EF
		else {
	...
	private void readBinary(APDU apdu, byte[] buffer) {
		...
		apdu.sendBytesLong(
			((ElementaryFile)selectedFile).getData(),
			offset,
			le);
	...
	/**
	 * change data in file that was selected with SELECT FILE
	 */
	private void updateBinary(APDU apdu, byte[] buffer) {
		...
		// impossible to start updating from offset large than max size of file
	...
	/**
	 * set this card ATR to the belpic card ATR
	 * This methed will throw an error if the card is not in select state or if 
	 * the applet is not default select and thus can not change historical ATR bytes.
	 * See GPSystem en Global Platform
	 */
	private void changeATR(APDU apdu) {
		
		// check if access to this file is allowed
		if (GPSystem.getCardContentState() != GPSystem.APPLICATION_SELECTABLE)
			ISOException.throwIt(ISO7816.SW_SECURITY_STATUS_NOT_SATISFIED);
		
		//Set the ATR of this card: TODO this only changes historical bytes, not full atr
		if (!GPSystem.setATRHistBytes(ATR, (short)0, (byte)ATR.length))
			ISOException.throwIt(ISO7816.SW_CONDITIONS_NOT_SATISFIED);//This is calles meaning that the atr can not be changed: returned false
		
	}

	/**
	 * checks if a certain file operation is allowed on the currently selected file
	 * 
	 * remark 1: a very dedicated (so not generic) implementation!
	 * ellegant solution would be to put (parts of) access control in File objects
	 * remark 2: there is a huge hack to allow some file updates
	 */
	private boolean fileAccessAllowed(byte mode) {
		...
		// we abuse the activation pin to update some of the large files (photo + certificates)
		if (GPSystem.getCardContentState() == GPSystem.APPLICATION_SELECTABLE)
			return true;
